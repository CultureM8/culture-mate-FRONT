...
const getBaseURL = () => {
  // 운영 환경: 환경 변수가 설정되어 있으면 우선 사용 (빌드 시점에 주입됨)
  if (process.env.NEXT_PUBLIC_BASE_URL && process.env.NEXT_PUBLIC_BASE_URL !== "http://localhost:8080") {
    return process.env.NEXT_PUBLIC_BASE_URL + "/api";
  }

  // 개발 환경: 브라우저에서는 Next.js 프록시 사용
