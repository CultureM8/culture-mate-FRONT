 ...
 */
export const toggleEventInterest = async (eventId, memberId) => {
  try {
    const response = await fetch(`${API_URL}/${eventId}/interest?memberId=${memberId}`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
...
// Named exports (기존 호환성 유지 + 새로운 메서드들)
export { 
  getEventById, 
  getEvents, 
  searchEvents, 
