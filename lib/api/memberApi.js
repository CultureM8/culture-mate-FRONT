...
const ENDPOINT = process.env.NEXT_PUBLIC_ENDPOINT_MEMBER || "/member";
...
const memberApi = {
  ...
  updateMemberStatus: async (memberId, status) => {
    try {
      const response = await fetch(`${API_URL}/${memberId}/status?status=${status}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: getHeaders(),
      });
  ...
  updateMemberRole: async (memberId, role) => {
    try {
      const response = await fetch(`${API_URL}/${memberId}/role?role=${role}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: getHeaders(),
      });
  ...
  updateMemberPassword: async (memberId, newPassword) => {
    try {
      const response = await fetch(`${API_URL}/${memberId}/password?newPassword=${encodeURIComponent(newPassword)}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: getHeaders(),
      });
