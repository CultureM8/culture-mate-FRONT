...
const validateTogetherRequest = (data) => {
  ...
  if (!data.address || data.address.length > 255) {
    errors.push('주소는 1-255자 이내여야 합니다');
  }
  
  if (!data.content || data.content.length > 2000) {
    errors.push('내용은 1-2000자 이내여야 합니다');
  }
...
const togetherApi = {
   ...
   * 새 동행 게시글 생성
   * @param {Object} newTogether - TogetherRequestDto 스키마 기반 데이터
   ...
   * @param {string} newTogether.address - 주소 (1-255자, 필수)
   * @param {string} newTogether.addressDetail - 상세 주소
   * @param {string} newTogether.meetingDate - 만남 날짜 (YYYY-MM-DD, 필수)
   * @param {number} newTogether.maxParticipants - 최대 참여 인원 (2-100, 필수)
   * @param {string} newTogether.content - 내용 (1-2000자, 필수)
