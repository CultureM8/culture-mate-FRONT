...
export default function FreeDonePage() {
  ...
  useEffect(() => {
    const p = getPost('free', postId);
    setPost(p || null);

    const onceKey = `viewed:free:${postId}`;
    if (p && !sessionStorage.getItem(onceKey)) {
      bumpViews('free', postId);
      sessionStorage.setItem(onceKey, '1');
      const updated = getPost('free', postId);
      setPost(updated || p);
    }

    setRecommended(isRecommended('free', postId));
  ...
  return (
    <>
      ...
      <div className="w-full max-w-[1200px]  mx-auto   ">
        ...
        <div className="flex justify-center my-6">
          <button
            ...
            onClick={() => {
              const ret = toggleRecommendation('free', postId);
      ...
      <ConfirmModal
        ...
        variant="danger"
        onConfirm={() => {
          deletePost('free', postId, { purgeExtras: true });
          setOpenDelete(false);
          // 목록으로 이동
